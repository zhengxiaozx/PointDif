...
def resume_model(base_model, args, logger = None):
    ...
    base_ckpt = {k.replace("module.", ""): v for k, v in state_dict['pointdif'].items()}
...
def save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, prefix, args, logger = None):
    if args.local_rank == 0:
        torch.save({
                    'pointdif' : base_model.module.state_dict() if args.distributed else base_model.state_dict(),
...
def load_model(base_model, ckpt_path, logger = None):
    ...
    # parameter load from ckpt
    if state_dict.get('pointdif') is not None:
        base_ckpt = {k.replace("module.", ""): v for k, v in state_dict['pointdif'].items()}
    else:
